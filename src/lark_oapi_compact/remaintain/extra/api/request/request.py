...
from ...api.api_error import (
    APIError,
    ERR_ACCESS_TOKEN_TYPE_INVALID,
    ERR_TENANT_KEY_IS_EMPTY,
    ERR_USER_ACCESS_TOKEN_KEY_IS_EMPTY,
    ERR_APP_TICKET_IS_EMPTY,
    ERR_HELP_DESK_AUTH_EMPTY,
)
...
class Request(Generic[T]):
    ...
    def prepare(self, config: Config):
        ...
        if option.tenant_key != "" and self.accessible_token_type_list.count(ACCESS_TOKEN_TYPE_TENANT):
            self.access_token_type = ACCESS_TOKEN_TYPE_TENANT
            self.tenant_key = option.tenant_key

        if option.user_access_token != "" and self.accessible_token_type_list.count(ACCESS_TOKEN_TYPE_USER):
    ...
    def do_request(self, config):  # type: (Config) -> requests.Response
        ...
        if isinstance(body, FormData):
            config.logger.debug(
                "request http_path:{}, http_method:{}, access_token_type:{}, request body:{}".format(
...
DEFAULT_HTTP_REQUEST_HEADERS = {"User-Agent": "oapi-sdk-python/%s" % VERSION}
...
class Handlers:
    ...
    def prepare(self):  # type: () -> None
        ...
        if not req.http_path.startswith("http"):
            if req.http_path.startswith("/open-apis/"):
                req.http_path = "{}{}".format(conf.domain, req.http_path)
            else:
                req.http_path = "{}/{}/{}".format(conf.domain, OAPI_ROOT_PATH, req.http_path)
    ...
    def validate_response(self):  # type: () -> None
        ...
        if req.is_response_stream:
            ...
            if resp.status_code != 200:
                raise APIError(
                    message="response status_code:%d, not equal 200, body:%s" % (resp.status_code, resp.content)
                )
            return
    ...
    def parse_response(self):  # type: () -> None
        ...
        self.config.logger.debug(
            "request http_path:%s, response status code:%d, response body:%s"
...
class AccessToken:
    ...
    def apply_app_ticket(self):  # type: () -> None
        ...
        req_body = ApplyAppTicketReq(config.app_settings.app_id, config.app_settings.app_secret)
    ...
    def set_app_access_token_to_store(self, access_token):
        ...
        key = "{}-{}".format(APP_ACCESS_TOKEN_KEY_PREFIX, conf.app_settings.app_id)
        try:
    ...
    def set_tenant_access_token_to_store(self, access_token):
        ...
        conf = self.handlers.config
        key = "{}-{}-{}".format(TENANT_ACCESS_TOKEN_KEY_PREFIX, conf.app_settings.app_id, tenant_key)
        try:
            conf.store.set(key, token, expire - EXPIRE_DELTA)
        except Exception as e:
            conf.logger.warn("storage tenant_access_token into storage occurred an error:%s" % e.__str__())

    def get_internal_app_access_token(self):  # type: () -> AppAccessToken
        conf = self.handlers.config
        internal_access_token_request = InternalAccessTokenReq(conf.app_settings.app_id, conf.app_settings.app_secret)
    ...
    def get_isv_app_access_token(self):
        ...
        isv_app_access_token_req = ISVAppAccessTokenReq(settings.app_id, settings.app_secret, app_ticket)
    ...
    def get_isv_tenant_access_token(self):
        ...
        isv_tenant_access_token_req = ISVTenantAccessTokenReq(app_access_token.app_access_token, tenant_key)
    ...
    def set_tenant_access_token(self):  # type: () -> None
        ...
        req.set_authorization_to_header(tenant_access_token.tenant_access_token)

    def get_app_ticket(self):  # type: () -> Tuple[bool, str]
        config = self.handlers.config
        store = config.store

        key = "{}-{}".format(APP_TICKET_KEY_PREFIX, config.app_settings.app_id)
    ...
    def get_internal_tenant_access_token(self):
        ...
        internal_access_token_req = InternalAccessTokenReq(settings.app_id, settings.app_secret)
...
def resolve_path(path, path_variables):  # type: (str, Dict[str, str]) -> str
    ...
    for i in range(len(splatted)):
        if splatted[i].startswith(":"):
            ...
            if v == "":
                raise RuntimeError("http path:{}, param name `{}` not found value".format(path, k))
            splatted[i] = v
